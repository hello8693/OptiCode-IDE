...
export class CompetitiveCompanionService {
  ...
  async importProblem(payload: CompetitiveCompanionPayload, workspaceDir: string): Promise<ICompetitiveCompanionImportResult> {
  ...
  private deriveBaseId(payload: CompetitiveCompanionPayload): string {
    ...
    const base = name || group || urlId || `problem-${Date.now()}`;
  ...
  private async buildMeta(payload: CompetitiveCompanionPayload, id: string, metaPath: string): Promise<IProblemMeta> {
